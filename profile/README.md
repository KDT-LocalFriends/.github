...
도메인 로직과 추천파이프라인은 프로세스 수준에서 분리되어 있어, 추천 서버 장애가 인증·상점 등 도메인 기능까지 멈추게 하지 않습니다.
...
- 추천 서버는 `bge-reranker-large` 가중치(약 2GB)를 인스턴스에서 직접 서빙하므로, 모델 로딩 메모리를 감안해 두 서버의 리소스 배분을 구성
- Unity 클라이언트가 EC2를 직접 호출하는 구조라 CORS를 서버 레벨에서 정리 — 게이트웨이나 프록시 없이도 클라이언트-서버 호출 경로를 단순하게 유지
- 배포 대상이 둘로 늘어난 비용을 받아들이는 대신, 수정 빈도가 높은 추천 파이프라인을 도메인 서버 재배포 없이 독립적으로 배포
